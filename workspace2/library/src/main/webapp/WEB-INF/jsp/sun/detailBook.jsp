...
/* function check1(){
	if(${sessionScope.usr==null || sessionScope.usr==""}){
		alert("회원만 로그인 가능합니다. 로그인 해주세요");
	}
} */
</script>
<style type="text/css">
		...
		<div id="libraryinfo" class="posts" >
  	  		<div style="float:left;">
  	  		 <div  style="width: 140px; height: 110px; overflow: hidden; margin-left:-20px;" class="col-sm-4 col-md-3 bookimg">
				    <div class="thumbnail">
				      <img src="images/book/${bookdto.bookImg}" alt="..." style="width: 110px; height: auto;">
				    </div>
				  </div>
	  	  
	  	  		<div >
	  	  			<p>도서이름 : ${bookdto.bookTitle}</p>
	  	  			<p>저자     : ${bookdto.bookWriter}</p>
	  	  			<p>출판사   : ${bookdto.bookCompany}</p>
	  	  			<p>청구번호 : ${bookdto.bookCallnumber}</p>
	  	  			<p style="padding-left:120px;">ISBN   :  ${bookdto.bookIsbn}</p>
	  	  			<p>줄거리</p>
	  	  			<p>${bookdto.bookContent}</p>
	  	  		</div>
	  	  		<div>
		  			  ...
		  			  <c:forEach items="${isbnlist}" var="isbnlist">
			  			  <tr>
			  				<td>${isbnlist.bookNo}</td>
			  				<td>${isbnlist.bookCallnumber}</td>
			  				<td>${isbnlist.rentState}</td>
			  				<td>${isbnlist.rentDueday}</td>
			  				<c:if test="${isbnlist.rentBooking=='예약가능'}">
			  			<!-- 	<td><input type="button" id="reserve" onclick="check1();" value="예약하기"></td> -->
			  					 <td><input type="button" id="reserve" onclick="location.href='reserve?bookno=${isbnlist.bookNo}';" value="예약하기"></td> 
			  				</c:if>
			  			  </tr>
		  			  </c:forEach>
