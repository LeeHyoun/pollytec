			  ...
			  <table class="table table-hover">
	  			  ...
	  			  <c:forEach items="${noticeList}"	var="noticeList">
		  			  <tr>
		  				...
		  				<td><a href="read?noticeseq=${noticeList.noticeseq}">${noticeList.noticetitle}</a></td>
		  				<td>${noticeList.mgrid}</td>
		  				<td>${noticeList.mgrname}</td>
		  				<td>${noticeList.noticedate}</td>
		  			  </tr>
	  			  </c:forEach>
	  			  
			  </table>
			  <input type="button" class="btn btn-default" onclick="location.href='writeform';" value="등록">
			   <!-- 페이지 처리 -->
			<div align="center">
				<div class="row">
			  	<div class="col-xs-12 col-sm-offset-3 col-sm-5 col-md-offset-3 col-md-5 col-lg-offset-3 col-lg-5">
			  		<nav>
			  			<ul class="pagination">
			  					<li><a href="managerMain?page=1" ><span aria-hidden="true">처음</span></a></li>
			   					<li><a href="managerMain?page=${prevpageno}" aria-label="Previous"><span aria-hidden="true">&laquo;</span></a></li>
			   				 <c:forEach begin="${gstart}" end="${gend}"   var="i">
			   				 	
			   				  		<li><a href="managerMain?page=${i}">${i}</a></li>
			   				  	
			   				</c:forEach>
			   				
			   				<li><a href="managerMain?page=${nextpageno}" aria-label="Next"><span aria-hidden="true">&raquo;</span></a></li>
			   				<li><a href="managerMain?page=${totalpage}" aria-label="Next"><span aria-hidden="true">마지막</span></a></li>
			 			 </ul>
					</nav>
				</div>
				</div>
